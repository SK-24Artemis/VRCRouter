...
namespace ValueFactoryVRCRouter {
  internal static class Program {
    ...
    static void Main() {
      ...
      // TODO exception
      var thread = new Thread(background_main);
      thread.Start();
    ...
    static void loud_crash(string text) {
      MessageBox.Show(text, "VRCRouter", MessageBoxButtons.OK, MessageBoxIcon.Error);
      Environment.Exit(1);
    }
    ...
    static void wrapped_main() {
      // NOTE(valuef): Idle until the form has been created
      // 2023-06-10
      while(!main.Created) {
        Thread.Sleep(100);
      }
      ...
      if(do_routing) {
        ...
        MessageBox.Show("Failed to start OSC Routing. Please check the console for more information.", "VRCRouter", MessageBoxButtons.OK, MessageBoxIcon.Warning);
      }
    }
  }
}
